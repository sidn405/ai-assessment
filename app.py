...
async def complete_game(request: CompleteGameRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # ── Award points: 10% of game score, capped at 10 points max ──────
        ...
        return {"success": True, "new_badges": [], "points_awarded": points_awarded}
...
@app.get("/api/admin/backfill-lexile-history")
async def backfill_lexile_history(admin=Depends(require_admin)):
    """
    ONE-TIME endpoint to backfill lexile_history rows for students whose
    Lexile score was set by the earlier one-time backfill (or otherwise
    predates history tracking). Without this, the admin Lexile Growth
    chart has nothing to plot even though users.lexile_score is correct.

    Replays each student's completed session_logs in chronological order
    through the same calculate_student_lexile() + rolling-average logic,
    inserting one lexile_history row per completion using the lesson's
    actual completed_at date, so the growth chart reflects real history.

    Only touches students who currently have zero lexile_history rows,
    so it's safe to run even if some students already have live-tracked
    ...
    cursor = get_cursor(conn)
    try:
        # Students with no lexile_history rows yet
        cursor.execute("""
        ...
        for uid in student_ids:
            ...
            # Keep users.lexile_score consistent with the final replayed value
            if USE_POSTGRES:
